...
const handleSnackClose = (_, reason) => {
  ...
  const validateForm = () => {
    const newErrors = {
      workModeName: !formData.workModeName.trim(),
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/work-modes`, {
        method: 'POST',
        headers: {
      ...
      //alert("Work mode successfully added");
      showSnack('Work mode successfully added', 'info');
      navigate('/work-mode-list');
    } catch (error) {
  ...
  return (
    <Container maxWidth="sm">
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4">Add New Work Mode</Typography>
      </Box>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <TextField
                ...
                required
                error={errors.workModeName}
                helperText={errors.workModeName && "This field is required"}
              />
            </Grid>
