...
const CurrencyList = () => {
  ...
  const handleDelete = async (idCurrency) => {
    try {
      await axios.delete(`${API_URL}/api/currencies/${idCurrency}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      setCurrencies(prev => prev.filter(currency => currency.idCurrency !== idCurrency));
    } catch (err) {
      setError("Erreur lors de la suppression");
    }
  };

  const filteredList = currencies.filter(c =>
  ...
  return (
    <Paper elevation={0} sx={{ backgroundColor: 'transparent', px: 10, py: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h4" fontWeight="bold">Currencies</Typography>
      </Box>
      ...
      {loading ? (
        ...
        <Typography align="center">No currencies found.</Typography>
      ) : (
        <>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 600 }}>Currency Name</TableCell>
              ...
              <TableBody>
                {filteredList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                  .map((currency, index) => (
                    <TableRow key={currency.idCurrency}>
                      <TableCell>{page * rowsPerPage + index + 1}</TableCell>
                      <TableCell>{currency.currencyName}</TableCell>
                      <TableCell>
                        <IconButton color="error" onClick={() => handleDelete(currency.idCurrency)}>
                          <DeleteIcon />
                        </IconButton>
                      </TableCell>
                    </TableRow>
